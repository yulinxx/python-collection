# python-opencv实现抖动算法_Hello_wshuo-CSDN博客_python 图像抖动
# https://wshuo.blog.csdn.net/article/details/114853678


import cv2
import numpy as np

img = cv2.imread("F:/p.jpg", 0)
...
for h in range(0, height, k_size):
    for w in range(0, width, k_size):
        gray = int(sum([img[h][w], img[h][w + 1], img[h + 1][w],
                        img[h + 1][w + 1]]) / (k_size * k_size))  # 计算2x2矩阵中灰度平均值

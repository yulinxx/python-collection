# canopy
# 美: [ˈkænəpi]  英: ['kænəpi] 
# n.	天篷；罩盖；顶篷；（尤指森林里）天篷似的冠层
# v.	用天篷遮覆 华盖；雨篷；遮篷

# https://github.com/AlanConstantine/CanopyByPython

# 机器学习算法：知道canopy、K-means++、二分K-means、K-medoids的优化原理_canopy算法-CSDN博客
# https://blog.csdn.net/itcast_cn/article/details/126061877
# Canopy算法的优缺点
# 优点：
# ​ 1.Kmeans对噪声抗干扰较弱，通过Canopy对比，将较小数量的簇直接去掉有利于抗干扰。
            ...
            while len(self.dataset) > 1:
                ...
                for j in range(len(self.dataset)):
                    ...
                    distance = self.euclideanDistance(
                        current_center, v)  # 计算选取的中心点P到每个点之间的距离
                    
                    if distance < self.t1:
                        # 若距离小于t1，则将点归入P点的canopy类
                        current_center_list.append(v)
                    if distance < self.t2:
                        delete_list.append(j)  # 若小于t2则归入删除容器

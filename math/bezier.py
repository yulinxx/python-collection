# （一）轨迹规划：贝塞尔曲线的python实现_我什么都布吉岛的博客-CSDN博客_python 贝塞尔曲线
    ...
    def DigitalAlgo(self):
        ...
        for u in np.arange(0, 1+1/self.num, 1/self.num):
            for i in range(0, self.pointsNum):
                PB[i] = (math.factorial(self.order)/(math.factorial(i) *
                         math.factorial(self.order-i)))*(u**i)*(1-u)**(self.order-i)*self.Points[i]
...
if show3D:
        ...
        for i in range(0,points3D.shape[0]):
            ax.scatter(points3D[i][0],points3D[i][1],points3D[i][2],marker='o',color='r')
            ax.text(points3D[i][0],points3D[i][1],points3D[i][2],i,size=12)

        # 直线连接控制点
        l=Line(points3D,1000)
        pl=l.getLinePoints()
        ax.plot3D(pl[:,0],pl[:,1],pl[:,2],color='k')

        # 贝塞尔曲线连接控制点
        bz=Bezier(points3D,1000)
